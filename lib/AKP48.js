...
function AKP48(config, configOpts, reloadFunction, startTime) {
  ...
  this.pluginTypes = {
    ...
    BackgroundTask: require('./BackgroundTaskPlugin'), // Deprecated - remove when plugins are updated
...
AKP48.prototype.loadPlugins = function () {
  ...
  return new Promise(function (resolve) {
    ...
    for (var name of Object.keys(genericPlugins)) {
      ...
      global.logger.verbose(`AKP48: Loading ${plugin.pluginName}`);
      try {
        plugin.load();
      } catch (e) {
        // Remove the plugin?
        return global.logger.error(`AKP48: Error while loading ${plugin.pluginName}.`, e);
      }
    }
